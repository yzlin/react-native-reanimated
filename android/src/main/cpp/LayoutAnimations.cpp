...
jni::local_ref<JMap<JString, JObject>> LayoutAnimations::getStyleWhileMounting(int tag, double progress, alias_ref<JMap<JString, JDouble>> values, int depth) {
    if (auto rt = this->weakUIRuntime.lock()) {
        jsi::Value layoutAnimationRepositoryAsValue = rt->global().getPropertyAsObject(*rt, "global").getProperty(*rt, "LayoutAnimationRepository");
        if (!layoutAnimationRepositoryAsValue.isUndefined()) {
...
jni::local_ref<JMap<JString, JObject>> LayoutAnimations::getStyleWhileUnmounting(int tag, double progress, alias_ref<JMap<JString, JDouble>> values, int depth) {
     if (auto rt = this->weakUIRuntime.lock()) {
        jsi::Value layoutAnimationRepositoryAsValue = rt->global().getPropertyAsObject(*rt, "global").getProperty(*rt, "LayoutAnimationRepository");
        if (!layoutAnimationRepositoryAsValue.isUndefined()) {
