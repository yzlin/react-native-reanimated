...
static jsi::Value __hostFunction_NativeReanimatedModuleSpec_forceRender(
    jsi::Runtime &rt,
    TurboModule &turboModule,
    const jsi::Value *args,
    size_t count)
{
  static_cast<NativeReanimatedModuleSpec *>(&turboModule)->forceRender(rt, std::move(args[0]));
  return jsi::Value::undefined();
}
...
NativeReanimatedModuleSpec::NativeReanimatedModuleSpec(std::shared_ptr<CallInvoker> jsInvoker)
  ...
  methodMap_["forceRender"] = MethodMetadata{
    1, __hostFunction_NativeReanimatedModuleSpec_forceRender};
}
